...
class Unet(nn.Module):
    def __init__(self, in_channels=4, base_channels=16):
        ...
        self.EnBlock5_3 = EnBlock(in_channels=base_channels * 16)
        self.EnBlock5_4 = EnBlock(in_channels=base_channels * 16)



    def forward(self, x):
        x = self.InitConv(x)       # (1, 16, 160, 192, 160)

        x1_1 = self.EnBlock1(x)
        x1_2 = self.EnDown1(x1_1)  # (1, 32, 80, 96, 80)

        x2_1 = self.EnBlock2_1(x1_2)
        x2_1 = self.EnBlock2_2(x2_1)
        x2_2 = self.EnDown2(x2_1)  # (1, 64, 40, 48, 40)

        x3_1 = self.EnBlock3_1(x2_2)
        x3_1 = self.EnBlock3_2(x3_1)
        x3_2 = self.EnDown3(x3_1)  # (1, 128, 20, 24, 20)

        x4_1 = self.EnBlock4_1(x3_2)
        x4_1 = self.EnBlock4_2(x4_1)
        x4_2 = self.EnDown4(x4_1)  # (1, 256, 10, 12, 10)

        x5_1 = self.EnBlock5_1(x4_2)
        x5_2 = self.EnBlock5_2(x5_1)
        x5_3 = self.EnBlock5_3(x5_2)
        output = self.EnBlock5_4(x5_3)  # (1, 256, 10, 12, 10)
